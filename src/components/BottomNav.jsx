...
const tabs = [
  { path: '/dashboard', label: 'Home', icon: (active) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? 'var(--accent)' : 'var(--text-muted)'}>
  ...
  { path: '/ai-coach', label: 'AI Coach', icon: (active) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? 'var(--accent)' : 'var(--text-muted)'} strokeWidth="2">
      <path d="M12 2a10 10 0 100 20 10 10 0 000-20z"/>
      <path d="M12 6v6l4 2"/>
      <circle cx="12" cy="12" r="3" fill={active ? 'var(--accent)' : 'var(--text-muted)'} stroke="none"/>
    </svg>
  )},
]
          ...
          >
            ...
            {active && (
              <span style={{
                ...
                animation: 'labelSlideUp 180ms ease-out forwards',
              }}>{tab.label}</span>
            )}
          </div>
        )
