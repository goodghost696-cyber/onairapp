...
export default function Scan() {
  ...
  // differs. A real in-camera toggle isn't possible here: this uses the
  // OS's own native camera UI (<input capture>), which we have zero control
  // over — no custom overlay can be injected into it. Same outcome via a
  // toggle BEFORE opening the camera instead: one "Scanner" button, one
  // small chip pair above it picks which mode it opens in.
  ...
  return (
    <div className="app-wrapper">
      <div className="screen">
        ...
        {/* Input buttons — always visible. "Décrire un repas" (multi-
            ingredient text description) moved to Nutrition.jsx's main
            screen, above Idée recette — not duplicated here. */}
        {!loading && !result && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {/* Mode toggle for the camera button below — was 2 separate
                full-width buttons (Photo/Code-barres) indistinguishable
                until tapped. Picking the mode BEFORE opening the camera
                (rather than inside it) is deliberate: this uses the
                device's own native camera UI (<input capture>), which
                can't have any custom overlay injected into it. */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <button type="button" className={`meal-chip${scanMode === 'food' ? ' active' : ''}`} onClick={() => setScanMode('food')} style={{ flex: 1, textAlign: 'center' }}>
                Repas
              </button>
              <button type="button" className={`meal-chip${scanMode === 'barcode' ? ' active' : ''}`} onClick={() => setScanMode('barcode')} style={{ flex: 1, textAlign: 'center' }}>
                Code-barres
              </button>
            </div>

            {/* Camera — opens directly in the mode picked above */}
            <label htmlFor="camera-input" className="scan-btn primary">
              {scanMode === 'barcode' ? <BarcodeIcon /> : <CameraIcon />}
              <div style={{ flex: 1 }}>
                <span style={{ display: 'block', fontSize: 13, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase' }}>
                  {scanMode === 'barcode' ? t('scan_barcode') : t('take_photo')}
                </span>
                <span className="scan-btn-sub">
            ...
            {/* Gallery — unaffected by the toggle above, always food-mode */}
