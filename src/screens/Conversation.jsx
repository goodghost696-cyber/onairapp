...
import '../styles/Conversation-redesign.css'
...
export default function Conversation({ isCoach = false }) {
  ...
  return (
    <div className={`app-wrapper conv-redesign${isCoach ? ' coach-narrow' : ''}`}>
      <div className="conv-header" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '20px 16px 12px', position: 'sticky', top: 0, zIndex: 10 }}>
        <button className="conv-back" style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => navigate(isCoach ? '/coach/messages' : '/messages')}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <div className="conv-avatar" style={{ width: 36, height: 36, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, flexShrink: 0 }}>
          {(member?.prenom?.[0] || '?').toUpperCase()}
        </div>
        <h1 className="text-base bold conv-title">{title}</h1>
      </div>
        ...
        {messages.map(m => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-end' : 'flex-start' }}>
              <div className={`conv-bubble ${isMine ? 'mine' : 'theirs'}`} style={{
                maxWidth: '75%', padding: '10px 14px', fontSize: 15, lineHeight: '1.4',
                borderRadius: isMine ? '18px 18px 4px 18px' : '18px 18px 18px 4px',
              }}>{m.content}</div>
              <span className="conv-time" style={{ fontSize: 10, marginTop: 2 }}>{formatTime(m.created_at)}</span>
            </div>
          )
        })}
        <div ref={bottomRef} />
      ...
      <div className="conv-input-row" style={{ position: 'fixed', bottom: 100, left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 32px)', maxWidth: 358, display: 'flex', gap: 8, alignItems: 'center', zIndex: 90, paddingTop: 8 }}>
        <input className="conv-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Message..." style={{ flex: 1, borderRadius: 24, padding: '12px 16px' }} />
        <button className="conv-send" onClick={send} disabled={!otherUserId} style={{ width: 40, height: 40, borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
