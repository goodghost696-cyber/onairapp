...
- [ ] **Bug trouvé en marge** : dans le "Programme IA" (`Workout.jsx`), le poids suggéré par l'IA peut disparaître une fois l'exercice ajouté à la séance — `addExercisesToSession()` (`AppContext.jsx`) fait `kg: ex.kg || ''`, qui transforme un poids **légitime de 0** (ex. exercice au poids du corps) en chaîne vide à cause du piège classique JS "zéro est falsy". Pas encore corrigé.
- [ ] **Validation des données chiffrées dans toute l'app** : l'utilisateur veut qu'on audite tous les champs numériques saisissables (onboarding poids/taille, objectifs, sheet d'édition Dashboard, etc.) pour empêcher des valeurs absurdes — référence directe au bug des 10800 kcal (session du 07-10, poids 454kg/taille 545cm jamais bloqués à la saisie). Pas encore audité.
- [ ] **Nutrition — pas de modification d'un repas après ajout** : on peut ajouter un repas mais pas corriger une erreur de saisie après coup (ex. 100g de skyr entré au lieu de 250g). La table `repas` n'a qu'une policy RLS insert/select/delete, pas d'update — solution la plus simple : permettre de supprimer puis ré-ajouter corrigé (UI à faire), plutôt que d'ajouter une vraie édition en place.
...
- [ ] **Run.jsx — retirer la carte "Dénivelé"** juste au-dessus du bouton start/stop, jugée moche et inutile (données factices, `elevationBars` codé en dur).
