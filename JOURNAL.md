...
- [ ] Auditer les autres tables (`objectifs`, `repas`, `seances`, `activite_jour`) pour une variante de ce même problème (GRANT table-large sur des colonnes qui ne devraient être modifiables par personne côté client) — pas trouvé cette fois mais pas vérifié aussi finement que sur `profiles`.
...
- Build + `node --check` sur les 5 fichiers `api/*` modifiés : tout passe. **Pas encore testé en conditions réelles sur la preview** (nécessite de déclencher volontairement la limite, ou au moins de vérifier qu'un usage normal — une recette IA, une recherche d'aliment — passe toujours).
