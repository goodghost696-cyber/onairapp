...
### Sprint sécurité 2 — dette (2026-07-17, 3/4 faits)
- [ ] **Activer "Leaked Password Protection" dans Supabase Auth** — seul point non fait, hors de portée des outils disponibles (réglage du service Auth, pas de la base Postgres — pas de tool MCP pour ça). **Action manuelle requise côté utilisateur** : Supabase Dashboard → Authentication → Policies → Password Security → activer.
