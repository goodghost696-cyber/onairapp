...
### Sprint design 1 — direction visuelle (proposé le 2026-07-17, pas encore cadré en détail)
- [ ] **Shader Landing** : intégrer un fond animé façon [ShaderGradient](https://shadergradient.co/customize?animate=on&axesHelper=off&bgColor1=%23000000&bgColor2=%23000000&brightness=1.2&cAzimuthAngle=180&cDistance=3.6&cPolarAngle=90&cameraZoom=1&color1=%23ff5005&color2=%23dbba95&color3=%23d0bce1&destination=onCanvas&embedMode=off&envPreset=city&format=gif&fov=45&frameRate=10&gizmoHelper=hide&grain=on&lightType=3d&pixelDensity=1&positionX=-1.4&positionY=0&positionZ=0&range=disabled&rangeEnd=40&rangeStart=0&reflection=0.1&rotationX=0&rotationY=10&rotationZ=50&shader=defaults&type=plane&uAmplitude=1&uDensity=1.3&uFrequency=5.5&uSpeed=0.4&uStrength=4&uTime=0&wireframe=false) — fond noir pur, plan 3D animé, gradient chaud (orange `#ff5005` → beige `#dbba95` → lavande `#d0bce1`), grain, rotation lente. Direction voulue par l'utilisateur : un mix entre l'esthétique Bevel (health-tech, données) et ON AIR, mais avec ce rendu abstrait/premium en plus — inspiré en discutant du positionnement face à [bevel.health](https://bevel.health) (voir Session 11 pour le comparatif complet). Recommandation de Claude à l'époque : réserver ce traitement aux écrans où l'ambiance prime sur la densité d'info — **Landing** en priorité, éventuellement les états de chargement IA (Idée recette, AI Coach) — pas sur Dashboard/Nutrition/Weekly où la lisibilité des données prime. Pas encore commencé : à choisir une implémentation (lib `@shadergradient/react` + `three`/`@react-three/fiber`, ou un shader GLSL custom plus léger vu que c'est un fond animé simple) avant de coder, en gardant un œil sur le poids ajouté au bundle (déjà un warning de chunk >500kB) et sur la batterie/perf mobile (l'app est testée sur iPhone en PWA).
...
- [ ] **(2026-07-16) "Mes charges" (`Weekly.jsx`) toujours en données fictives** (Bench Press/Squat/Deadlift/Pull-up hardcodés) — maintenant que les séances persistent réellement (étape 3, `seances.exercices` jsonb contient les vrais poids/reps par set), on peut en extraire une vraie courbe de progression par exercice. Pas encore fait.
...
- [ ] **(2026-07-16) ⚠️ Carte "Dénivelé"/circuit factice toujours visible — j'avais corrigé le mauvais fichier.** Il existe **deux implémentations séparées et dupliquées** de l'écran Course : `src/screens/Run.jsx` (route `/run`, déclarée dans `App.jsx` mais **jamais liée nulle part** — aucun bouton/lien n'appelle `navigate('/run')`, donc cet écran n'est normalement jamais vu) et `src/components/RunContent.jsx` (affiché à l'intérieur de `Workout.jsx` quand on clique l'onglet "COURSE" — **c'est celui-là que l'utilisateur voit réellement**, confirmé par capture d'écran). J'ai retiré la carte "Dénivelé" de `Run.jsx` (le mauvais fichier, mort) ; `RunContent.jsx` n'a pas de section "Dénivelé" à proprement parler, mais a un **circuit map factice** (tracé de route coloré inventé, juste au-dessus du bouton start) qui est very probablement l'élément visé par la demande initiale ("la carte poubelle c'est trop moche"). **Pas encore corrigé.** À faire à la prochaine session : retirer le circuit map de `RunContent.jsx` (pas de `Run.jsx`), et évaluer si `Run.jsx`/la route `/run` doit être supprimée puisqu'elle semble être du code mort dupliqué.
