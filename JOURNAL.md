...
- [x] **Nutrition — modification d'un repas après ajout** (sprint 1, item 2) : `repas` n'ayant pas de policy RLS update (seulement insert/select/delete), implémenté en "supprimer puis ré-ajouter" plutôt qu'une édition en place — nouveau `deleteMeal()` dans `AppContext.jsx` + bouton supprimer avec confirmation inline sur chaque carte de repas.
...
- [x] **Run.jsx — carte "Dénivelé" retirée** (sprint 1, item 3) — était juste au-dessus du bouton start/stop, données factices (`elevationBars` codé en dur).
